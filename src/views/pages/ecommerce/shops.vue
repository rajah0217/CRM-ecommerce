...
<template>
  <Layout>
    ...
    <BRow>
      <BCol sm="6" cols="xl-4" v-for="(item, index) in shops" :key="index">
        <BCard no-body>
          <BCardBody>
            <div class="media d-flex align-items-start">
              <div class="avatar-sm me-4">
                <span
                  :class="`avatar-title bg-${item.color}-subtle text-${item.color} font-size-16 rounded-circle`"
                >
                  {{ item.name.charAt(0) }}
                </span>
              </div>
              <div class="media-body flex-grow-1 align-self-center">
